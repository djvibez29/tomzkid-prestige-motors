...
# ---------------- AUTO CREATE ADMIN + SAFE MIGRATIONS ----------------
...
def buy(vehicle_id):
    ...
    order = Order(buyer_email=current_user.email, vehicle_id=vehicle.id, amount=vehicle.price,
                  commission=commission, dealer_earnings=dealer_earnings, status="pending")
...
def admin_add():
    ...
    vehicle = Vehicle(title=request.form["title"], price=int(request.form["price"]),
                      year=int(request.form["year"]), mileage=int(request.form["mileage"]),
                      description=request.form.get("description", ""), image_url=image_url,
                      images=None, dealer_id=current_user.id, is_approved=True)
